    ...
    try {
      const result = await pool.query(`
        SELECT
          ...
          (SELECT COUNT(*)::int FROM retail_orders WHERE status='COMPLETED' AND (id_verified_at IS NULL OR completed_in_store=FALSE OR COALESCE(NULLIF(completion_reference,''),NULLIF(pos_receipt_number,'')) IS NULL)) incomplete_completed_orders,
